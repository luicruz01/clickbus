...
class Withdraw(Resource):
        ...
        if accounts and len(accounts) > 0:
            for account in accounts:
                ...
                # credit
                    ...
                    if 0 <= args['amount'] <= account[0]*1.1:
    ...
    @staticmethod
    def update_amount(user_id, account_id, amount, withdraw):
        query = """
            UPDATE Account SET amount = {amount}
            WHERE ac.user_id = {user_id}
            AND ac.account_id = {account_id}
        """
        return conn.execute(query=query.format(amount=amount-withdraw, user_id=user_id, account_id=account_id), commit=True)

    @staticmethod
    def update_amount_credit_card(user_id, account_id, amount, debt, withdraw):
        query = """
                    UPDATE Account SET amount = {amount}, debt = {debt}
                    WHERE ac.user_id = {user_id}
                    AND ac.account_id = {account_id}
                """
        return conn.execute(query=query.format(amount=amount-withdraw, user_id=user_id, account_id=account_id, debt=debt+(withdraw*1.1)), commit=True)
